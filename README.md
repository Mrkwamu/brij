...
The keys are never stored in plain text, only an HMAC-SHA256 is stored, so when the database is breached it never exposes the user's secrets. Every verify call checks the key status, expiry, rotation deadline and rate limit in a single request, with Redis caching to keep latency low.

## Features

 - **Key issuance** - create structured API keys with a prefixname, prefix, environment (live/test) and cryptographic secret
 - **Secure hashing** - keys are stored as HMAC-SHA256 hashes and not raw plain text
 ...
 - **Rate limiting** — per-key request limits with token bucket via Redis
...
- OTP delivery handled by a BullMQ background job so registration doesn't block on email delivery
- JWT access token + refresh token issued
- Refresh token stored in httpOnly cookie never exposed to JavaScript
- Full JWT rotation on every refresh, old refresh token is invalidated, new pair issued
